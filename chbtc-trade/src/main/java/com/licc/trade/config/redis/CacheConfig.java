 ...
 * 
 * import com.licc.trade.util.Collections3; import java.lang.reflect.Method;
 * 
 ...
 * 
 * @Bean public KeyGenerator wiselyKeyGenerator(){ return new KeyGenerator() {
 * 
 ...
 * JedisConnectionFactory factory = new JedisConnectionFactory( new
 * RedisClusterConfiguration(Collections3.transStringToList(nodes,","))); return
 * factory; }
 * 
 * @Bean public CacheManager cacheManager(RedisTemplate redisTemplate) {
 * RedisCacheManager cacheManager = new RedisCacheManager(redisTemplate); //
 * Number of seconds before expiration. Defaults to unlimited (0)
 * cacheManager.setDefaultExpiration(10); //设置key-value超时时间 return cacheManager;
 * }
 * 
 * @Bean public RedisTemplate<String, String>
 * redisTemplate(RedisConnectionFactory factory) { StringRedisTemplate template
 * = new StringRedisTemplate(factory); setSerializer(template);
 * //设置序列化工具，这样ReportBean不需要实现Serializable接口 template.afterPropertiesSet();
 * return template; } private void setSerializer(StringRedisTemplate template) {
